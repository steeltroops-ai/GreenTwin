...
export class CommunityLeaderboardSystem {
  ...
  private generateMockLeaderboard() {
    const mockNames = [
      'EcoExplorer', 'GreenGuru', 'CarbonCrusher', 'SustainableSam', 'ClimateChamp',
      'EarthGuardian', 'NatureNinja', 'PlanetPal', 'EcoWarrior', 'GreenMachine',
      'CarbonCutter', 'SolarSage', 'WindWalker', 'TreeHugger', 'OceanProtector',
      'RecycleRanger', 'BikeBuilder', 'SolarSaver', 'WaterWise', 'EnergyExpert'
    ];
  ...
  private generateMockAchievements(totalSaved: number, streak: number): string[] {
  ...
  private generateWeeklySavings(): number[] {
    return Array.from({ length: 7 }, () => Math.round((5 + Math.random() * 10) * 10) / 10);
  }
  ...
  private calculateRankings() {
    ...
    const recentEntries = sortedEntries
      .filter(entry => Date.now() - entry.last_activity < 30 * 24 * 60 * 60 * 1000)
      .sort((a, b) => {
        const aWeeklyTotal = a.weekly_savings.reduce((sum, val) => sum + val, 0);
        const bWeeklyTotal = b.weekly_savings.reduce((sum, val) => sum + val, 0);
  ...
  getLeaderboard(type: 'all_time' | 'monthly' = 'all_time', limit: number = 10): LeaderboardEntry[] {
  ...
  getUserRank(userId: string = 'current_user'): { all_time: number; monthly: number } | null {
  ...
  updateUserProgress(co2Saved: number, category: string = 'general') {
    ...
    const daysSinceLastActivity = Math.floor((Date.now() - this.userEntry.last_activity) / (24 * 60 * 60 * 1000));
    if (daysSinceLastActivity <= 1) {
      this.userEntry.current_streak = Math.max(this.userEntry.current_streak, daysSinceLastActivity === 0 ? this.userEntry.current_streak : this.userEntry.current_streak + 1);
      this.userEntry.longest_streak = Math.max(this.userEntry.longest_streak, this.userEntry.current_streak);
  ...
  private checkAchievements(entry: LeaderboardEntry): Achievement[] {
    ...
    this.achievements.forEach(achievement => {
      ...
      switch (achievement.criteria.type) {
        case 'co2_saved':
          earned = entry.total_co2_saved >= (achievement.criteria.threshold || 0);
          break;
        case 'streak':
          earned = entry.longest_streak >= (achievement.criteria.threshold || 0);
          break;
        case 'category':
  ...
  getLeaderboardStats(): LeaderboardStats {
    ...
    const totalSaved = entries.reduce((sum, entry) => sum + entry.total_co2_saved, 0);
    const topStreak = Math.max(...entries.map(entry => entry.longest_streak));
    const totalAchievements = entries.reduce((sum, entry) => sum + entry.achievements.length, 0);
