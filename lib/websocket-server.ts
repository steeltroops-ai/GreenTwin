...
export class GreenTwinWebSocketServer {
  ...
  private setupServer() {
    try {
      ...
      console.log(`Green Twin WebSocket server running on port ${this.port}`);
    } catch (error) {
  ...
  private determineClientType(userAgent: string, origin: string): 'extension' | 'web' | 'unknown' {
    if (userAgent.includes('Chrome') && !origin) {
      return 'extension';
    } else if (origin && (origin.includes('localhost') || origin.includes('vercel.app'))) {
      return 'web';
    }
    return 'unknown';
  }
