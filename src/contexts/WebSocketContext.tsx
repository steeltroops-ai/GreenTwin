...
export function WebSocketProvider({ children }: WebSocketProviderProps) {
  ...
  const handleEventMessage = (message: WebSocketMessage) => {
    ...
    if (data.source === "extension") {
      ...
      if (
        ...
        data.type === "predictive_intervention"
      ) {
  ...
  const handlePreferenceMessage = (message: WebSocketMessage) => {
    ...
    setPreferences((prev: any) => ({ ...prev, ...message.data }));
  };

  const handleSyncResponse = (message: WebSocketMessage) => {
    if (message.data.events) {
      console.log(
        "Green Twin: Syncing events from server:",
  ...
  if (recentEvents.length === 0) {
    return (
      <div className="text-center text-muted-foreground py-8">
        ...
        <p className="text-sm">
          Extension events will appear here in real-time
        </p>
      </div>
    );
  }
